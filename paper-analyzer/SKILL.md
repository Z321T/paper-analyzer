---
name: paper-analyzer
description: Comprehensive academic paper analysis for AI reading, reviewing, and presentation preparation. Use when the user mentions analyzing a paper, reading a paper, understanding a paper, preparing a presentation about a paper, reviewing a paper, or comparing papers. Triggers: "分析论文", "读论文", "paper analysis", "论文", "paper review", "审稿", "组会", "presentation", "paper comparison", "论文对比", "速读", "精读", ".pdf" with research context. Supports any language paper (English, Chinese, bilingual) and outputs in the user's query language.
---

# Paper Analyzer

General-purpose academic paper analysis skill. Reads PDF and/or markdown papers and produces structured analysis across three reading modes with automatic diagram generation.

## Mode Selection

At the start of every session, detect the user's intent from their query:

| User says | Mode | Description |
|:----------|:-----|:------------|
| "速读" / "skim" / "快速" / "大概" / "overview" / "brief" | **Skim** | Quick scan, ~800 words, core message only |
| "精读" / "deep" / "详细" / "组会" / "presentation" / "深入" / no explicit mode | **Deep** | Full analysis, all 7 sections + diagrams |
| "审稿" / "review" / "批判" / "找问题" / "peer review" | **Review** | Critical review with Summary/Strengths/Weaknesses/Questions |
| 2-3 papers mentioned together with "对比" / "compare" / "区别" | **Comparison** | Side-by-side comparison across dimensions |

If mode is ambiguous, default to **Deep**.
...
- **[references/output-specifications.md](references/output-specifications.md)** — Complete output templates for Skim, Deep, Review, and Comparison modes. **Read this for every analysis** to follow the exact section structure.
...
- **[references/mermaid-templates.md](references/mermaid-templates.md)** — Mermaid diagram templates: mindmap, method flowchart, training pipeline, memory architecture, experiment pipeline, comparison matrix, decision flow. **Read this when generating any diagram.**
...
- Don't confuse mode requirements: Skim should be short, Deep should be exhaustive, Review should be critical
